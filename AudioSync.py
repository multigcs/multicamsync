#
# from https://github.com/allisonnicoledeal/VideoSync.git
#
...
class AudioSync:
	...
	def extract_audio(self, video_file):
		output = video_file + ".wav"
		if not os.path.isfile(output):
			print("extract audio: " + video_file + " -> " + output)
	...
	def fourier(self, sample):  #, overlap):
		mag = []
	...
	def align(self, video1, video2, fft_bin_size=1024, overlap=0, box_height=512, box_width=43, samples_per_box=7):
		# Process first file
		wavfile1 = self.extract_audio(video1)
		raw_audio1, rate = self.read_audio(wavfile1)
		bins_dict1 = self.make_horiz_bins(raw_audio1[:44100*120], fft_bin_size, overlap, box_height) #bins, overlap, box height
		boxes1 = self.make_vert_bins(bins_dict1, box_width)  # box width
		ft_dict1 = self.find_bin_max(boxes1, samples_per_box)  # samples per box

		# Process second file
		wavfile2 = self.extract_audio(video2)
		raw_audio2, rate = self.read_audio(wavfile2)
		bins_dict2 = self.make_horiz_bins(raw_audio2[:44100*60], fft_bin_size, overlap, box_height)
		boxes2 = self.make_vert_bins(bins_dict2, box_width)
		ft_dict2 = self.find_bin_max(boxes2, samples_per_box)

		# Determie time delay
		pairs = self.find_freq_pairs(ft_dict1, ft_dict2)
